...
export async function requireAdminSession(request: Request): Promise<AdminSession> {
  ...
  if (!accessToken) {
    throw new AdminAuthError(401, 'UNAUTHENTICATED', 'Admin login is required.')
  }
  ...
  if (userError || !user) {
    throw new AdminAuthError(401, 'INVALID_SESSION', 'Admin session is invalid or expired.')
  }
  ...
  if (!adminUser) {
    throw new AdminAuthError(403, 'FORBIDDEN', 'Admin permission is required.')
  }

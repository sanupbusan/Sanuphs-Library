...
- `ADMIN_LOGIN_ID`: 관리자 로그인 화면에서 입력할 아이디입니다. 기본값은 `SanupLib`입니다.
- `ADMIN_AUTH_EMAIL`: Supabase Auth에 실제로 생성할 관리자 사용자 이메일입니다. 앱은 `ADMIN_LOGIN_ID`를 이 이메일 로그인으로 매핑합니다.
...
- access token 만료 후에는 다시 로그인해야 합니다.
